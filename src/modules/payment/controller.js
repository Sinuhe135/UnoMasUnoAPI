...
const {getAllPaymentsAdmin,getAllPayments,checkTeacherPayment,getPayment,getPaymentIdTeacher,editPayment,createPayment, deletePayment} = require('../../databaseUtils/payment.js');
...
async function getAll(req,res)
{
    try {
        ...
        {
            payments = await getAllPaymentsAdmin();
        }
        else if(res.locals.type === 'independiente' || res.locals.type === 'general')
        {
            payments = await getAllPayments(res.locals.idAuth);
        }

        response.success(req,res,payments,200);
    } catch (error) {
